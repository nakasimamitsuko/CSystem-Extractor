...
- Delta 图像自动合并（内存缓存基础图，基于 mask 位图差分还原）
- 可选 C 原生加速库（LZSS 解压 **150x**、压缩 **2000x** 加速）
- 无 C 编译器时自动回退纯 Python，零依赖即可运行

## 项目结构

```
CSystem-Extractor/
├── csystem_tool.py   # 主工具 (Python)
├── lzss_fast.c       # LZSS C 加速库源码
├── build_lzss.py     # 一键编译脚本
└── README.md
```
...
编译需要 C 编译器：
- **Windows**: Visual Studio 开发者命令行（`cl`）或 MinGW（`gcc`）
- **Linux**: `sudo apt install gcc`

编译后生成 `lzss_fast.dll`（Windows）或 `lzss_fast.so`（Linux），放在 `csystem_tool.py` 同目录下，运行时自动加载。

> 不编译也能用，只是慢。工具会自动检测并提示。

### 3. 使用

```bash
# 解包图像（Arc02 = 索引，Arc05 系列 = 内容）
...
| 操作 | 纯 Python | C 加速 | 加速比 |
|------|----------|--------|-------|
| LZSS 解压 | 469 ms | **3 ms** | 150x |
| LZSS 压缩 | ~60 s | **30 ms** | 2000x |

C 加速库使用哈希链表优化匹配查找（替代 Python 版的 4096 窗口暴力搜索），编译时启用 `-O3` 优化。
...
> 部分游戏的 Arc02 同时索引音频+图像+配置，全部内容存储在同一个 Arc05.dat 中。

## Delta 图像处理

C,system 引擎使用差分压缩来存储 CG 变体（表情差分、衣装差分等）。工具会自动处理：

1. **完整图（type 'a'）**: 直接保存为 PNG
2. **Delta 图（type 'd'）**: 内存中查找 `base_index` 指向的基础图，通过 mask 位图合并后保存
3. **标准包装（type 'c'）**: 内部是标准 BMP/JPEG/PNG，直接提取

Delta 图和基础图会被保存到以基础图 ID 命名的子文件夹中：
...
C 加速库 `lzss_fast.c` 在压缩端使用哈希链表（3 字节滑动窗口哈希 + 链长限制 128）替代暴力搜索，将 O(n×4096) 降至近似 O(n)，实测压缩加速约 2000 倍。
...
| `FUN_00427a20` | LZSS 解压 | `lzss_decompress` / `lzss_fast.c` |
...
类型 b/c/e/n/o/p 的数据经过 LZSS 压缩（`CompressedSize < UncompressedSize`），其余类型为原始数据（`CompressedSize == UncompressedSize`）。
